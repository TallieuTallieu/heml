...
const { isPlainObject, defaults, mapValues, castArray, compact, flattenDeep } =
  lodash;
...
export default function (name, attrs, ...contents) {
  ...
  if (contents.filter(isPromise).length > 0) {
    return Promise.all(contents).then((contents) =>
      render(name, attrs, contents.join("")),
    );
  }
...
function render(name, attrs, contents) {
  if (!name || (isPlainObject(name) && !name.render)) {
    throw new Error(
      `name must be a HEML element or HTML tag name (.e.g 'td'). Received: ${JSON.stringify(name)}`,
    );
  }
