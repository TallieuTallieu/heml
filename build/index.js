...
function parse(contents, options = {}) {
  ...
  const selfClosingTags = [...selfClosingHtmlTags, ...elements.filter(element => element.children === false).map(({
    tagName
  }) => tagName)];
  ...
  const $head = lodash.first(lodash.compact([...$('head').toNodes(), ...$('body').toNodes(), ...$('heml').toNodes()]));
